...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect_password = []
    def withdraw_with_password(amount,_password):
        nonlocal balance,password,incorrect_password
        if len(incorrect_password) >= 3:
            return "Too many incorrect attempts. Attempts: %s" % str(incorrect_password)
        if _password != password:
            incorrect_password.append(_password)
...
def merge(incr_a, incr_b):
    ...
    while next_a != None or next_b != None:
        if next_a == next_b:
            yield next_a
            next_a, next_b = next(iter_a, None), next(iter_b, None) 
        if next_b == None or (next_a != None and next_b != None and next_a < next_b):
            yield next_a
