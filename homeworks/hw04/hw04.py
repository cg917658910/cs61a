...
def balanced(m):
    ...
    if is_planet(m):
        ...
        ok = length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
    return ok and balanced(end(left(m))) and balanced(end(right(m)))
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    assert is_mobile(m), "must a mobile or a planet"
...
def replace_leaf(t, find_value, replace_value):
    ...
    def f(tr):
        if is_leaf(tr):
            if label(tr) == find_value:
                tr = tree(replace_value)
            return tr
        return tree(label(tr),[f(b) for b in branches(tr)])
    return f(t)
