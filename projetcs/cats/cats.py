...
def choose(paragraphs, select, k):
    ...
    return '' if k > len(ans) -1 else ans[k]
...
def about(topic):
    ...
    return lambda p: len([word for word in topic if word in split(lower(remove_punctuation(p)))]) > 0
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    typed_words_count,reference_words_count = len(typed_words),len(reference_words)
    percentage = 0.0 # If typed is empty, then the accuracy is zero.
    correctly_count = 0
    if typed_words_count > 0:
        for i in range(min(typed_words_count, reference_words_count)):
            print("Debug: ",typed_words[i],reference_words[i])
            if typed_words[i] == reference_words[i]:
                correctly_count += 1
        percentage = correctly_count / typed_words_count * 100.0
    return percentage
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    diff_function_memo = (diff_function)
    diff_words = [[valid_word,diff_function_memo(user_word,valid_word,limit)] for valid_word in valid_words]
    
    similar_word,similar_diff = min(diff_words,key=lambda item: item[1])

    return user_word if similar_diff > limit else similar_word
...
def shifty_shifts(start, goal, limit):
    ...
    return (val + shifty_shifts(start[1:],goal[1:],limit - val))
...
def report_progress(typed, prompt, id, send):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    assert len(prompt) > 0, 'prompt must be non-empty'
    correctly_count = 0
    for i in range(len(typed)):
        if typed[i] != prompt[i]:
            break
        correctly_count += 1
    progress = correctly_count / len(prompt)
...
def fastest_words(game):
    ...
    """
    players = (all_times(game))  # An index for each player
    words = (all_words(game))    # An index for each word
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    words_times = [[] for i in range(len(words))]
    fastest_player = [[] for i in range(len(players))]
    for i in range(len(words)):
        for j in range(len(players)):
            player = players[j]
            words_times[i].append([player[i]])
        player_index = min(range(len(players)),key=lambda player_index: words_times[i][player_index])
        fastest_player[player_index].append(words[i])
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    diff_function_memo = memo(diff_function)
    diff_words = [[valid_word,diff_function_memo(user_word,valid_word,limit)] for valid_word in valid_words]
