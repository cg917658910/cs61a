...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # find the nearest place that is not the HIVE
        curr_range,place = 0,self.place
        bees = []
        # min_range
        while curr_range < self.min_range and place is not beehive:
            curr_range,place = curr_range + 1,place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bee_amount = amount if amount < self.armor else amount + self.damage
        for bee in bees:
            Insect.reduce_armor(bee, bee_amount)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return self.contained_ant is None and isinstance(other,Ant) and not isinstance(other,ContainerAnt)
        # END Problem 9
        ...
        if not self.true_queen:
            ...
            while place.exit is not None:
                 ...
                 if place.ant is not None:
                    if not place.ant.has_buffed_damage:
                        place.ant.damage *= 2
                        place.ant.has_buffed_damage = True
                    if isinstance(place.ant,ContainerAnt) \
                        and place.ant.contained_ant is not None \
                        and not place.ant.contained_ant.has_buffed_damage:
                        place.ant.contained_ant.damage *= 2
                        place.ant.contained_ant.has_buffed_damage = True
